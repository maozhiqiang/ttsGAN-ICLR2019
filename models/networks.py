...
def discriminator(inputs_content, inputs_mel, is_training, scope='discriminator'):
...
def style_encoder(inputs, filters, kernel_size, strides, is_training, scope='style_encoder'):
 with tf.variable_scope(scope, reuse=tf.AUTO_REUSE):
   ref_outputs = tf.expand_dims(inputs,axis=-1)
   features = {}
   # CNN stack
   for i, channel in enumerate(filters):
     ref_outputs = conv2d(ref_outputs, channel, kernel_size, strides, tf.nn.relu, is_training, 'conv2d_%d' % i)
     features['conv2d_%d'%i] = ref_outputs
